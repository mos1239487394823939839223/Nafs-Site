...
export default function DailyAgenda() {
  ...
  const dailyBookings = bookings.filter((b) => {
    ...
    const bookingDate = new Date(b.SessionStartTime)
      .toISOString()
      .split("T")[0];
  ...
  // Sort by time
  dailyBookings.sort(
    (a, b) => new Date(a.SessionStartTime) - new Date(b.SessionStartTime),
  );
  ...
  const getSlotColor = (type, priority) => {
    if (type === "break" || type === "off")
      return "bg-background-subtle border-border opacity-60";
  ...
  return (
    <div
      className="bg-background-paper rounded-2xl shadow-sm p-6 border border-border"
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div
      ...
      >
        <div className={isRTL ? "text-right" : "text-left"}>
          <h2 className="text-xl font-bold text-text-heading">
            {t("doctor.dailyAgenda", "Daily Agenda")}
          </h2>
          <p className="text-sm text-text-muted mt-1" dir="auto">
                    ...
                    >
                      ...
                      <span className="text-sm font-medium">
                        {slot.label === "Lunch Break"
                          ? t("common.lunchBreak", "Lunch Break")
                          : t("common.break", "Break")}
                      </span>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      {slot.duration && (
                        <p className="text-xs text-text-muted">
                          {slot.duration} {t("common.min", "min")}
                        </p>
                      )}
                    </div>
                  )}
                ...
                {slot.type === "appointment" && (
                  <div className="flex-shrink-0">
                    <Button
                      size="sm"
                      variant={
                        slot.priority === "urgent" ? "primary" : "outline"
                      }
                      onClick={() => {
                        if (slot.meetingUrl) {
      ...
      >
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            ...
            <div className="text-xs text-text-muted mt-1">
              {t("doctor.availableSlots", "Available Slots")}
            </div>
          </div>
          <div>
