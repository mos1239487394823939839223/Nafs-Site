...
// Reusing the robust security settings design from Doctor, as security needs are similar
            ...
            <div className="bg-gradient-to-br from-primary/5 to-secondary/5 border border-primary/10 rounded-2xl p-6 relative overflow-hidden">
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-start gap-4">
                        <div className={`p-3 rounded-xl ${twoFactor ? 'bg-green-100 text-green-600' : 'bg-background-subtle text-text-muted'}`}>
                            <Shield className="w-8 h-8" />
                        </div>
                        <div>
                            <h3 className="font-bold text-lg text-text-heading">Two-Factor Authentication</h3>
                            <p className="text-sm text-text-muted mt-1 max-w-md">
                                Secure your health data. We'll send a code to your phone when you log in.
                            </p>
                            <div className="mt-3 flex items-center gap-2">
                                <span className={`w-2 h-2 rounded-full ${twoFactor ? 'bg-green-500' : 'bg-red-500'}`} />
            ...
            <section>
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold text-lg text-text-heading">Password</h3>
                            <p className="text-xs text-text-muted">Last changed 1 year ago</p>
                        </div>
                    </div>
                    {!showPasswordForm && (
                ...
                <AnimatePresence>
                    {showPasswordForm && (
                        ...
                        >
                            <div className="bg-background-subtle border border-border rounded-2xl p-6 space-y-4 max-w-lg">
                                <Input
                                    type="password"
