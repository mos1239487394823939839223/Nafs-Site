...
export const HeroCard = ({ doctorName }: HeroCardProps) => {
  ...
  return (
    <section className="relative mb-7 overflow-hidden rounded-[30px] border border-border shadow-[0_18px_50px_-30px_rgba(15,76,58,0.35)]">
      ...
      <div className="absolute -top-20 end-[30%] h-56 w-56 rounded-full bg-secondary/20 blur-3xl" />
      <div className={`relative flex min-h-[300px] flex-col items-stretch ${isRTL ? "md:flex-row-reverse" : "md:flex-row"}`}>
        <div className="relative z-[3] flex flex-1 flex-col justify-center p-6 text-center sm:p-8 md:p-10 md:text-start lg:p-12">
          {doctorName && (
            <p className="mb-3 inline-flex items-center justify-center gap-2 text-xs font-bold text-secondary md:justify-start">
              <Sparkles className="h-4 w-4" />
              {t("doctor.dashboardHome.hero.greeting")},{" "}
              <span>{doctorName}</span>
            </p>
          )}
          <h1 className="mb-3 text-2xl font-black leading-[1.45] text-text-heading sm:text-3xl lg:text-[34px]">
            {t("doctor.dashboardHome.hero.titleLine1")}
            <br />
            {t("doctor.dashboardHome.hero.titleLine2")}
          </h1>
          <p className="mb-6 max-w-xl text-sm font-medium leading-7 text-text-light md:text-base">
          ...
          <div>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-primary px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-primary/20 hover:-translate-y-0.5 hover:bg-primary-dark"
            >
