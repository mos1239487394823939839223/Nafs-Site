...
export const HeroCard = ({ isNewPatient = false }: { isNewPatient?: boolean }) => {
  ...
  return (
    <section
      className="relative overflow-hidden rounded-[30px] border border-[#F3F4F6] shadow-[0_18px_45px_rgba(15,81,50,0.08)]"
      dir="ltr"
    >
      ...
      <div className="relative grid min-h-[300px] items-stretch md:grid-cols-[45%_55%] lg:grid-cols-2">
        <div className="relative flex min-h-[240px] items-center justify-center overflow-hidden md:min-h-[300px]">
          ...
          />
          <div className="pointer-events-none absolute inset-y-0 right-[-2px] z-[2] w-[42%] bg-gradient-to-r from-transparent via-[#F9F8F3]/70 to-[#fdfcfa]" />
        </div>

        <div className="relative z-[3] flex min-h-[260px] flex-col justify-center px-8 py-10 text-left md:min-h-[300px] md:px-12 lg:px-16">
          <h1 className="mb-5 max-w-[560px] text-[30px] font-black leading-[1.12] text-[#0B3D2E] sm:text-[36px] lg:text-[42px]">
            {isNewPatient ? t("patientHome.hero.newPatientTitle") : t("patientHome.hero.title")}
          </h1>
          <p className="mb-9 text-base font-medium leading-7 text-[#4B5563] sm:text-lg">
