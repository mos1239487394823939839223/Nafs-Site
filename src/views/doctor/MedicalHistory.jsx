...
export default function MedicalHistory() {
  ...
  bookings.forEach((booking) => {
    ...
    if (patientId && !seenPatients.has(patientId)) {
      ...
      const completedSessions = patientBookings.filter((b) => b.Status === 3)
        .length;
      ...
      myPatients.push({
        ...
        lastSessionDate: lastSession?.SessionStartTime
          ? new Date(lastSession.SessionStartTime).toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
              year: "numeric",
            })
  ...
  const handleUpdateResult = async (record) => {
    ...
    try {
      const response = await medicalAPI.updatePatientTestResult(
        record.id,
        value,
      );
      if (response?.IsSuccess === false) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto min-h-screen">
        ...
        {!selectedPatient ? (
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl font-bold mb-2 text-text-heading">
                  {t("doctor.medicalHistory")}
                </h1>
                <p className="text-text-muted">
                  {t("doctor.manageClinicalRecords")}
                </p>
              </div>
            ...
            {filteredPatients.length === 0 ? (
              <div className="text-center py-20 bg-background-paper rounded-3xl border-2 border-dashed border-border">
                ...
                <h3 className="text-xl font-bold text-text-heading mb-2">
                  {t("doctor.noPatientsFound")}
                </h3>
                <p className="text-text-muted">
                  {t("doctor.patientsAppearHere")}
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-lg text-text-heading truncate">
                          {patient.name}
                        </h3>
                        <p className="text-sm text-text-muted">
                          {patient.completedSessions} completed • Last:{" "}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <h2 className="text-xl md:text-2xl font-bold text-text-heading truncate">
                    {selectedPatient.name}
                  </h2>
                  <p className="text-text-muted text-sm">
                    {t("doctor.clinicalHistory")} •{" "}
                    {selectedPatient.totalSessions} {t("doctor.sessions")}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              {historyLoading ? (
                ...
                patientHistory.map((record) => (
                  ...
                  >
                    ...
                    <div className="p-6 space-y-4">
                      <div>
                        <h4 className="text-xs font-black italic uppercase tracking-widest text-text-muted mb-2">
                          {t("doctor.sessionSummary")}
                        </h4>
                        <p className="text-text-heading leading-relaxed">
                          {record.testTypeName}
                        </p>
                        {!!record.examNotes && (
                      ...
                      {record.medications && record.medications.length > 0 && (
                        <div>
                          <h4 className="text-xs font-black italic uppercase tracking-widest text-text-muted mb-2">
                            {t("doctor.prescribedMedications")}
                          </h4>
                ...
                <div className="text-center py-20 bg-background-paper rounded-3xl border-2 border-dashed border-border">
                  ...
                  <h3 className="text-xl font-bold text-text-heading mb-2">
                    {t("doctor.noClinicalRecords")}
                  </h3>
                  <p className="text-text-muted">
                    {t("doctor.addSessionNote")}
                  </p>
                </div>
              )}
            </div>
      ...
      >
        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-black italic text-text-muted uppercase tracking-tighter">
              Test Type
            </label>
