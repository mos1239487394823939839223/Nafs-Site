...
const mockArticles = [
  {
    ...
    summary: 'Learn the steps to ensure your virtual consultation goes smoothly, from testing your connection to organizing your concerns beforehand.',
    summaryAr: 'تعرف على الخطوات اللازمة لضمان سير استشارتك الافتراضية بسلاسة، من اختبار الاتصال إلى تنظيم استفساراتك مسبقًا.',
    content: 'A telehealth visit can be just as effective as an in-person appointment when done right. Here are the key steps to prepare...',
    contentAr: 'يمكن أن تكون زيارة الرعاية الصحية عن بُعد فعالة مثل المواعيد الشخصية عند إعدادها بالشكل الصحيح...',
  ...
  },
  {
    ...
    summary: 'Mental health is just as important as physical health. This guide helps you understand common conditions, symptoms, and when to seek help.',
    summaryAr: 'الصحة النفسية لا تقل أهمية عن الصحة الجسدية. يساعدك هذا الدليل على فهم الحالات الشائعة والأعراض ومتى تطلب المساعدة.',
    content: '',
    contentAr: '',
  ...
  },
  {
    ...
    summary: 'We\'ve added several exciting features to improve your experience, including enhanced appointment scheduling and faster load times.',
    summaryAr: 'أضفنا عدة ميزات مثيرة لتحسين تجربتك، بما في ذلك جدولة مواعيد محسّنة وأوقات تحميل أسرع.',
    content: '',
    contentAr: '',
    author: 'Technical Support Team',
    authorAr: 'فريق الدعم التقني',
  ...
  },
  {
    ...
    summary: 'Quality sleep is fundamental to mental wellness. Discover evidence-based strategies to improve your sleep habits and overall wellbeing.',
    summaryAr: 'النوم الجيد أساسي للصحة النفسية. اكتشف استراتيجيات قائمة على الأدلة لتحسين عادات نومك وصحتك العامة.',
    content: '',
    contentAr: '',
  ...
  },
  {
    ...
    summary: 'Step-by-step guide on how to communicate with your healthcare team through the secure messaging feature on our platform.',
    summaryAr: 'دليل خطوة بخطوة حول كيفية التواصل مع فريق الرعاية الصحية الخاص بك من خلال ميزة المراسلة الآمنة على منصتنا.',
    content: '',
    contentAr: '',
    author: 'Technical Support Team',
    authorAr: 'فريق الدعم التقني',
...
function ArticleCard({ article, isRTL, isBookmarked, onToggleBookmark, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {/* Title */}
        <h3 className={`font-bold text-text-heading text-base leading-snug group-hover:text-primary transition-colors mb-2 ${isRTL ? 'text-right' : 'text-left'}`}>
          {isRTL ? article.titleAr : article.title}
        </h3>

        {/* Summary */}
        <p className={`text-sm text-text-muted line-clamp-2 leading-relaxed mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
          {isRTL ? article.summaryAr : article.summary}
        </p>

        {/* Tags */}
        <div className={`flex flex-wrap gap-1.5 mb-4 ${isRTL ? 'justify-end' : 'justify-start'}`}>
          {(isRTL ? article.tagsAr : article.tags).slice(0, 3).map((tag, i) => (
            <span key={i} className="text-xs px-2 py-0.5 bg-background-subtle border border-border rounded-full text-text-muted">
              #{tag}
            </span>
          ))}
        </div>
        ...
        {/* Footer */}
        <div className={`flex items-center justify-between gap-2 text-xs text-text-muted border-t border-border pt-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <span>{new Date(article.publishedAt).toLocaleDateString(isRTL ? 'ar-EG' : 'en-US', { month: 'short', day: 'numeric' })}</span>
          </div>
        </div>
      </div>
    </motion.div>
        ...
        >
          ...
          <div className="p-6" dir={isRTL ? 'rtl' : 'ltr'}>
            <div className={`flex items-start justify-between gap-4 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <button onClick={onClose} className="text-text-muted hover:text-text transition-colors text-2xl leading-none">&times;</button>
            </div>

            <h2 className={`text-2xl font-bold text-text-heading mb-3 ${isRTL ? 'text-right' : 'text-left'}`}>
              {isRTL ? article.titleAr : article.title}
            </h2>

            <div className={`flex items-center gap-4 text-sm text-text-muted mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <Circle className="w-1 h-1" />
              <span>{new Date(article.publishedAt).toLocaleDateString(isRTL ? 'ar-EG' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </div>

            <div className={`text-text-muted leading-relaxed mb-6 ${isRTL ? 'text-right' : 'text-left'}`}>
              <p className="mb-4 text-base">{isRTL ? article.summaryAr : article.summary}</p>
              ...
              {!(isRTL ? article.contentAr : article.content) && (
                <div className="p-6 bg-background-subtle rounded-xl text-center text-text-muted">
                  ...
                  <p className="text-sm">{isRTL ? 'المحتوى الكامل سيكون متاحاً قريباً' : 'Full content coming soon'}</p>
                </div>
              )}
            </div>
            ...
            <div className={`flex flex-wrap gap-2 ${isRTL ? 'justify-end' : 'justify-start'}`}>
              {(isRTL ? article.tagsAr : article.tags).map((tag, i) => (
                <span key={i} className="flex items-center gap-1 text-xs px-3 py-1 bg-primary/10 text-primary rounded-full font-medium">
...
export default function PatientArticles() {
  ...
  const filtered = mockArticles.filter(a => {
    const matchSearch = search === '' || (isRTL ? a.titleAr : a.title).toLowerCase().includes(search.toLowerCase())
    const matchFilter = filter === 'all' ? true
      : filter === 'saved' ? bookmarks.has(a.id)
      : a.category === filter
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
        <div>
          ...
          <p className="text-text-muted mt-1">
            {isRTL ? 'مقالات ودلائل من فريق الدعم والأطباء' : 'Articles and guides from support team & doctors'}
          </p>
        </div>
        {/* Search */}
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 bg-background-paper border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-text transition-all`}
          />
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <h3 className={`font-bold text-lg leading-snug mb-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                    {isRTL ? article.titleAr : article.title}
                  </h3>
                  <p className={`text-white/80 text-sm line-clamp-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                    {isRTL ? article.summaryAr : article.summary}
                  </p>
                  <div className={`flex items-center gap-3 mt-4 text-white/70 text-xs ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <span>{isRTL ? article.authorAr : article.author}</span>
                    <Circle className="w-1 h-1" />
                    <span>{article.readTime} {isRTL ? 'دقائق' : 'min'}</span>
                  </div>
                </div>
              </motion.div>
            )
        ...
        {filters.map(tab => (
          ...
          >
            ...
            {tab.key === 'saved' && bookmarks.size > 0 && (
              <span className="ms-2 bg-primary text-white text-xs rounded-full px-1.5 py-0.5">{bookmarks.size}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="text-center py-20 bg-background-subtle/20 rounded-2xl border-2 border-dashed border-border">
          ...
          <p className="text-text-muted mt-2 text-sm">
            {isRTL ? 'جرب تغيير فلتر البحث' : 'Try changing your search or filter'}
          </p>
        </div>
      )}
