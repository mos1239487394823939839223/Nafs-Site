...
function BlogCard({ blog, onClick, t, isRTL }) {
  ...
  return (
    <motion.div
      ...
      tabIndex={0}
      aria-label={
        isRTL ? `فتح المقال ${blog.title}` : `Open article ${blog.title}`
      }
      ...
      <div className="p-5 space-y-3">
        ...
        >
          <div className="flex-1 min-w-0">
            <h3
              className={`font-bold text-text-heading text-base leading-snug group-hover:text-primary transition-colors line-clamp-2 ${
        ...
        >
          {blog.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="text-xs px-2.5 py-0.5 bg-background-subtle border border-border rounded-full text-text-muted flex items-center gap-1"
            >
          ...
          {blog.tags.length > 3 && (
            <span className="text-xs px-2 py-0.5 bg-background-subtle border border-border rounded-full text-text-muted">
              +{blog.tags.length - 3}
            </span>
          )}
        </div>
...
function FeaturedCard({ blog, onClick, t, isRTL }) {
  return (
    <motion.div
      ...
      aria-label={
        isRTL
          ? `فتح المقال المميز ${blog.title}`
          : `Open featured article ${blog.title}`
      }
      onClick={() => onClick(blog)}
      onKeyDown={(event) => {
        if (event.key === "Enter" || event.key === " ") {
      ...
      >
        ...
        >
          ...
          <div className="p-6 space-y-5">
            ...
            <h2
              className={`text-2xl font-black text-text-heading leading-snug ${
                isRTL ? "text-right" : "text-left"
              }`}
            >
              {blog.title}
            ...
            <div
              className={`flex items-center gap-3 text-sm text-text-muted flex-wrap ${
            ...
            <div
              className={`text-text leading-relaxed text-base whitespace-pre-line ${
                isRTL ? "text-right" : "text-left"
              }`}
            >
              {blog.description}
            ...
            >
              {blog.tags.map((tag) => (
                <span
                  key={tag}
                  className="flex items-center gap-1 text-sm px-3 py-1 bg-primary/10 text-primary rounded-full font-medium border border-primary/20"
                >
...
export default function PatientBlogs() {
  ...
  useEffect(() => {
    ...
    toast.error(
      isRTL
        ? "فشل تحميل المقالات من الخادم"
        : "Failed to load blogs from server",
    );
        ...
        >
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              } py-3 bg-background-paper border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-text transition-all`}
            />
            {search && (
              <button
                onClick={() => setSearch("")}
                ...
                } top-1/2 -translate-y-1/2 text-text-muted hover:text-text`}
              >
