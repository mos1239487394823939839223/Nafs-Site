    ...
    >
      <div className="space-y-5" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <div>
          ...
          <input
            className={`w-full px-4 py-3 rounded-xl border bg-background text-text focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all text-sm ${
              errors.title
                ? "border-red-400"
        ...
        <div>
          ...
          {form.images.length > 0 ? (
            ...
            <label
              className={`flex flex-col items-center justify-center border-2 border-dashed border-border rounded-xl p-8 cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all text-sm ${
        ...
        <div>
          ...
          <div className={`flex gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
            ...
            >
              <option value="">
                {isRTL ? "اختر وسم من القائمة" : "Choose a tag from list"}
              </option>
              {allTags
                .filter(
                  (tag) =>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          className={`text-sm text-text-muted line-clamp-2 leading-relaxed ${
...
export default function AdminBlogs() {
  ...
  const handleCreateTag = async (name) => {
    ...
    try {
      ...
      toast.success(
        isRTL ? "تم إنشاء الوسم بنجاح" : "Tag created successfully",
      );
      await loadTags();
    } catch (err) {
      toast.error(
        extractErrorMessage(
          err,
          isRTL ? "فشل إنشاء الوسم" : "Failed to create tag",
        ),
      );
    } finally {
